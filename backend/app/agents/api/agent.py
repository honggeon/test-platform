...
7. **生成测试代码** → 基于测试用例生成可执行的测试脚本
   **重要：生成 Playwright 测试脚本时，必须使用 `allure-api-fixture.ts` fixture：**
   ```typescript
   // ✅ 正确写法
   import { test, expect } from '../allure-api-fixture';
   
   // ❌ 错误写法（缺少请求/响应详情）
   // import { test, expect } from '@playwright/test';
   ```
   这样每个 API 调用的请求 URL、请求体、响应状态码、响应体都会自动记录到 Allure 报告中。
...
|| 🌐 测试环境 | `get_default_environment` | 获取项目的默认测试环境 URL |
|| 🌐 环境列表 | `list_environments` | 查看项目所有可用测试环境 |
