...
async def fetch_openapi_from_url(url: str) -> dict[str, Any]:
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            ...
            # 检测是否为 HTML 页面（如 Swagger UI）
            if "text/html" in content_type or body_text.strip().startswith("<"):
                detail = (
                    f"URL 返回的是 HTML 页面（{content_type}），不是 JSON 文档。\n"
                    "请确认输入的是 OpenAPI JSON 文件的直链地址，而不是 Swagger UI 页面地址。\n"
            ...
            try:
                ...
                # JSON 解析失败，检测是否为 YAML 格式
                stripped = body_text.strip()
                if stripped.startswith("openapi:") or stripped.startswith("swagger:"):
                    detail = (
                        "文档是 YAML 格式，但当前仅支持 JSON 格式的 OpenAPI 文档。\n"
                        "请提供 JSON 格式的文档（.json 后缀），或将 YAML 转换为 JSON 后再上传。"
                    )
                else:
                    preview = body_text[:200].replace("\n", "\\n")
                    detail = (
                        f"URL 返回的内容不是有效的 JSON（content-type: {content_type}）。\n"
                        f"内容预览: {preview}"
                    )
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="获取远程文档超时（30秒），请检查 URL 是否可访问或尝试其他网络环境"
        )
    except httpx.RequestError as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"无法连接到远程服务器: {str(e)}"
        )
